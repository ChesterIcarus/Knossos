...
class apn_verification(object):
    ...
    def parsing_apns(self, filepath, database):
        ...
        for bounding in maz_set['features']:
            ...
            if (progress >= (.1*total)):print("1")
            if (progress >= (.2*total)):print("2")
            if (progress >= (.3*total)):print("3")
            if (progress >= (.4*total)):print("4")
            if (progress >= (.5*total)):print("5")
            if (progress >= (.6*total)):print("6")
            if (progress >= (.7*total)):print("7")
            if (progress >= (.8*total)):print("8")
            if (progress >= (.9*total)):print("9")
            progress += 1
